...
if (!empty($_POST)) {
    list('name' => $name, 'surname' => $surname, 'username' => $username, 'password' => $password, 'again_password' => $again_password) = $_POST;
    ...
    if (!empty($user)) {
    ...
    } else if (mb_strlen($username) === 0 || mb_strlen($username) > 10) {
        CreateCookie('is_invalid_username', true);
        ...
        if (5 < strlen($password) && strlen($again_password) < 10) {
            ...
            $add_user_query = "INSERT INTO users (name, surname, username, avatar, password, status) VALUES ('$name', '$surname', '$username', DEFAULT, '$hash', DEFAULT)";
